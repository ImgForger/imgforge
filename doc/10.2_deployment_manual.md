...
imgforge is a single binary that links against libvips at runtime, so wherever you run it, libvips has to be there too — which is the main reason the container image is the easier path. This covers container builds, systemd, reverse proxies, and hardening.

For automated deployment scripts, see [Automated Deployment](10.1_deployment_automated.md).
...
- `/status` and `/metrics` both respond.
- A signed URL for a representative transformation returns the image you expect — not just a `200`.
- The cache directory is writable, and entries survive a restart.
- `status_codes_total` shows no `4xx`/`5xx` step change during the rollout.
