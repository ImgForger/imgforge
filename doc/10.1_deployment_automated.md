...
It installs Docker if it is missing, checks the ports are free, then asks two questions: which caching strategy to use (memory, disk, hybrid, or none) and whether to bring up Prometheus and Grafana alongside imgforge.

From there it is unattended — it pulls the image, generates HMAC keys, writes the configuration, provisions the Grafana dashboard if monitoring was enabled, starts everything through Docker Compose, and health-checks the result before printing the URLs and credentials.
...
```bash
# Port Configuration
IMGFORGE_BIND=0.0.0.0:3000  # Main HTTP listener
...
It detects the distribution and package manager, installs libvips and the basic tooling, and downloads the release binary matching the machine's architecture (amd64 or arm64) into `/opt/imgforge/`. It asks the same two questions as the Docker script — caching strategy, and whether to install Prometheus and Grafana.

It then generates HMAC keys, writes `/etc/imgforge/imgforge.env`, creates the systemd units, enables them at boot, starts them, and health-checks before printing the URLs and credentials.
