...
### 5. Document Preset Inventory

Maintain a reference document or config file that lists all presets:

```yaml
# presets.yaml (documentation only, not consumed by imgforge)
presets:
  thumbnail:
    options: "resize:fit:150:150/quality:80"
    description: "Grid view thumbnails"
    used_in: ["product listing", "search results"]
  
  hero:
    options: "resize:fit:1920:1080/quality:92/sharpen:1.0"
    description: "Homepage hero images"
    used_in: ["landing pages", "campaign banners"]
```
...
### URL Builder (JavaScript)

```javascript
class ImgForgeURL {
  constructor(baseUrl, key, salt) {
  ...
  sign(path) {
    ...
    return generateSignature(this.key, this.salt, path);
  }

  build(sourceUrl, options = {}) {
    const encoded = base64UrlEncode(sourceUrl);
    let path = '';

    if (options.preset) {
      path += `/pr:${options.preset}`;
    }
...
// Usage
const forge = new ImgForgeURL('https://imgforge.example.com', KEY, SALT);
const url = forge.build('https://example.com/photo.jpg', {
  preset: 'thumbnail',
  format: 'webp'
});
// => https://imgforge.example.com/<sig>/pr:thumbnail/format:webp/<encoded>
```
...
class ImgForgeURL:
    def __init__(self, base_url, key, salt):
        self.base_url = base_url
        self.key = bytes.fromhex(key)
        self.salt = bytes.fromhex(salt)

    def sign(self, path):
        mac = hmac.new(self.key, self.salt + path.encode(), hashlib.sha256)
        return base64.urlsafe_b64encode(mac.digest()).rstrip(b'=').decode()

    def build(self, source_url, preset=None, **options):
        encoded = base64.urlsafe_b64encode(source_url.encode()).rstrip(b'=').decode()
        parts = []

        if preset:
            parts.append(f'pr:{preset}')
...
# Usage
forge = ImgForgeURL('https://imgforge.example.com', KEY, SALT)
url = forge.build('https://example.com/photo.jpg', preset='thumbnail', format='webp')
# => https://imgforge.example.com/<sig>/pr:thumbnail/format:webp/<encoded>
```
...
function useImgForge(baseUrl, key, salt) {
  return useMemo(() => {
    return {
      url(sourceUrl, preset, options = {}) {
        // Build URL with preset and options
        return buildImgForgeURL(baseUrl, key, salt, sourceUrl, preset, options);
      }
    };
  }, [baseUrl, key, salt]);
}
...
function ProductImage({ imageUrl }) {
  const forge = useImgForge(IMGFORGE_URL, IMGFORGE_KEY, IMGFORGE_SALT);
  
  return (
    <img
      src={forge.url(imageUrl, 'product_main')}
      srcSet={`
        ${forge.url(imageUrl, 'product_thumb')} 300w,
        ${forge.url(imageUrl, 'product_main')} 800w,
        ${forge.url(imageUrl, 'product_zoom')} 2000w
      `}
      sizes="(max-width: 768px) 300px, (max-width: 1200px) 800px, 2000px"
      alt="Product"
    />
  );
}
...
Preset expansion is extremely fast (< 1µs). The HashMap lookup and string parsing add negligible overhead compared to image processing.
...
### Preset Expansion Overhead

Memory and CPU impact of preset expansion is minimal:

- **Memory:** HashMap of preset definitions (~1KB typical)
- **CPU:** String splitting and HashMap lookup (~0.5µs per request)
- **Network:** Shorter URLs save bytes (e.g., `pr:thumbnail` vs `resize:fit:150:150/quality:80`)
...
If an attacker gains access to environment variables, they could define malicious presets:

```bash
# Malicious: extremely large output
IMGFORGE_PRESETS="evil=resize:fit:99999:99999/quality:100"
```

**Mitigations:**

1. Protect environment variable access (restrict server access, use secret management)
2. Set `IMGFORGE_MAX_SRC_RESOLUTION` to limit output size
3. Monitor resource usage (CPU, memory, disk)
4. Use `IMGFORGE_ALLOW_SECURITY_OPTIONS=false` to prevent per-request limit overrides

### Presets-Only Mode for Public APIs

Public-facing imgforge instances should strongly consider presets-only mode:
...
- Creating resource-intensive transformations
- Probing for vulnerabilities via crafted options
- Bypassing rate limits by varying option parameters
- Generating unlimited cache entries

## Migration Strategies

### Gradual Preset Adoption

**Phase 1:** Define presets alongside existing URL generation
...
Both styles work:
```
/<sig>/pr:thumbnail/<url>           # new
/<sig>/resize:fit:150:150/quality:80/<url>  # old
```

**Phase 2:** Update application code to use presets

```diff
- const url = buildUrl(source, { resize: 'fit:150:150', quality: 80 });
+ const url = buildUrl(source, { preset: 'thumbnail' });
```

**Phase 3:** Enable presets-only mode after full migration
...
Maintain backward compatibility during preset updates:

```bash
# Original
thumbnail=resize:fit:150:150/quality:80

# Updated, add versioned preset
thumbnail_v2=resize:fit:200:200/quality:85

# Migrate application code from 'thumbnail' to 'thumbnail_v2'
# Remove 'thumbnail' after full migration
```
